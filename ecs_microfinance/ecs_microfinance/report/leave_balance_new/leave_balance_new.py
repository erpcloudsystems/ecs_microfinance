...
def get_data(filters: Filters) -> List:
    ...
    active_employees = frappe.get_list(
        "Employee",
        filters=conditions,
        fields=["name", "employee_name", "department", "user_id", "leave_approver" , "branch"],
    )
    frappe.msgprint(str(active_employees))
    ...
    for leave_type in leave_types:
        ...
        for employee in active_employees:
            ...
            row.leaves_expired_date = get_expired_date(employee.name, leave_type) 
            leaves_taken = (
            ...
            row.indent = 1
            if row.leaves_allocated:
                data.append(row)
    return data

def get_expired_date(employee: str, leave_type: str) -> datetime:
    new_date = None  

    if frappe.db.exists("Leave Allocation", {"employee": employee, "leave_type": leave_type}):
        try:
            leave_docs = frappe.get_all("Leave Allocation", {"employee": employee, "leave_type": leave_type})
            for doc in leave_docs:
                leave_doc = frappe.get_doc("Leave Allocation", doc)
                if leave_doc.carry_forward == 1:
                    leave_type_doc = frappe.get_doc("Leave Type", leave_type)
                    expire_from_date = leave_doc.from_date
                    days_to_add = leave_type_doc.expire_carry_forwarded_leaves_after_days - 1
                    new_date = expire_from_date + timedelta(days=days_to_add)

        except Exception as e:
            frappe.log_error(f"Error while getting expired date: {str(e)}")
