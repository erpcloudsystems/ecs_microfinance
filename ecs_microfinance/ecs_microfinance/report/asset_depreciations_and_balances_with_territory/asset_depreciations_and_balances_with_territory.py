    ...
    for asset_category in asset_categories:
        ...
        row.update(
            next(
                ...
                for asset in assets
                if (
                    asset["asset_category"] == asset_category.get("asset_category", "")
                    and (not filters.territory or asset["territory"] == filters.territory)
                )
            )
        )
...
def get_asset_categories(filters):
    query = """
        ...
        from `tabAsset`
        where docstatus=1 and workflow_state!="Cancelled" and company=%(company)s and purchase_date <= %(to_date)s
    """
    if filters.territory:
        query += " and territory = %(territory)s"

    query += " group by asset_category, territory"
...
def get_assets(filters):
    query = """
        ...
        from (SELECT a.asset_category,
            ...
            where a.docstatus=1 
            and a.workflow_state!="Cancelled" and a.company=%(company)s and a.purchase_date <= %(to_date)s and a.name = ds.parent and ifnull(ds.journal_entry, '') != ''
    """
    if filters.territory:
        query += " and a.territory = %(territory)s"

    query += """
            group by a.asset_category, a.territory
            union
            ...
            from `tabAsset` a
            where a.docstatus=1
            and a.workflow_state!="Cancelled" and a.company=%(company)s and a.purchase_date <= %(to_date)s
    """
    if filters.territory:
        query += " and a.territory = %(territory)s"

    query += " group by a.asset_category, a.territory) as results group by results.asset_category, results.territory"
